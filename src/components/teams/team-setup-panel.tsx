...
import { createTeamAction, deleteTeamAction } from "@/app/(dashboard)/tournaments/[tournamentId]/teams/team-actions";
          ...
          {teams.length === 0 ? (
            ...
            <div className="space-y-3">
              {teams.map((team) => (
                <div
                  key={team.id}
                  className="glass-card glass-card-hover rounded-2xl p-4 flex items-center gap-4 group"
                >
                  {/* Logo */}
                  ...
                  {!auctionStarted && (
                    <button
                      ...
                      className="h-8 w-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition disabled:opacity-50"
                      title="Delete team"
                    >
